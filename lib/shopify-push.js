...
export function buildDraftOrderInput(order) {
  ...
  // Session-13 snapshots carry the engine's label ("Standard shipping + COD
  // fee ₹49"); older ones only had { method } — fall back gracefully.
  const shippingTitle =
    snap.shipping?.label ||
...
const ORDER_FIELDS = `id name displayFinancialStatus totalPriceSet { shopMoney { amount } }
  lineItems(first: 20) { nodes { quantity variant { id inventoryQuantity } } }`;
...
export async function pushOrderToShopify(orderId, { gql = adminGraphql } = {}) {
  ...
  try {
    ...
    // Inventory v1 (Session 18): Shopify is the source of truth and drafts
    // complete even past zero stock — ACCEPT the order, FLAG the oversell.
    const oversold = (shopifyOrder.lineItems?.nodes || []).filter(
      (li) => li.variant && li.variant.inventoryQuantity < 0,
    );
    if (oversold.length) {
      slog("oversell_flagged", {
        order_id: orderId,
        variants: oversold.map((li) => ({ id: li.variant.id, at: li.variant.inventoryQuantity })),
      });
      await sendAlert(`Oversell on ${orderId}: ${oversold.length} variant(s) below zero after this order`, {
        orderId,
        shopifyName: shopifyOrder.name,
        variants: oversold.map((li) => `${li.variant.id} @ ${li.variant.inventoryQuantity}`),
      });
    }

    await completeShopifySync(orderId, { shopifyOrderId: shopifyOrder.id });
    slog("shopify_push_synced", {
      order_id: orderId,
    ...
    slog("shopify_push_failed", { order_id: orderId, attempt, dead, error: msg.slice(0, 200) });
    if (dead) {
