...
const TRANSITIONS = {
  ...
  // paid → cancelled = refund territory: scripts/cancel-order.js only allows
  // it with a Razorpay refund id recorded (v1 refunds are manual — docs/refunds.md)
  paid: ["syncing_shopify", "cancelled"],
...
// Guest order-status links (Session 18): the status page is public but the
// order id alone must not be enough (BL-XXXXXXXX is short) — links carry an
// HMAC token derived from a server secret. No accounts in v1.
export async function orderStatusToken(orderId) {
  const secret =
    process.env.ORDER_LINK_SECRET || process.env.RAZORPAY_KEY_SECRET || "dev-only-secret";
...
export async function verifyOrderStatusToken(orderId, token) {
  if (!token) return false;
  return (await orderStatusToken(orderId)) === String(token);
}

export const orderStatusPath = async (orderId) =>
  `/orders/${orderId}?t=${await orderStatusToken(orderId)}`;
